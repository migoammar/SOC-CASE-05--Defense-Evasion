...
Hypothesis
IF: Account Name shows user01 and ComputerName shows PC01.example.corp. THEN: This event indicates the Security Event Log was cleared, suggesting an attacker attempted to cover their tracks by removing audit evidence. SO: Search Splunk using index=main EventCode=1102 | table _time Host Message.

Timeline
Uploaded the EVTX file to Splunk
Queried index=main source="*1102*" | stats count by EventCode
Queried index=main EventCode=1102 | table _time Host Message
Found suspicious behavior (EventCode 1102 — Security log cleared)
Built the hypothesis based on the identified evidence (Account Name, ComputerName, EventCode)
